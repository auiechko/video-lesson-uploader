...
9. `reconciliation.py` перевіряє в Telegram лише фактично перервані або
   невідомі доставки. `PENDING` не змінюється. Явно підтверджене користувачем
   повне скидання повертає всі записи поточного batch у `PENDING`, включно
   із `SENT`; перед цим створюється SQLite backup.
